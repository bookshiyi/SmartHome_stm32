...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagrelay_4)
    {
        info->event[info->num] = EVENT_relay_4;
        info->num++;
        dataPoints->valuerelay_4 = gizStandardDecompressionValue(relay_4_BYTEOFFSET,relay_4_BITOFFSET,relay_4_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagrelay_2)
    {
        info->event[info->num] = EVENT_relay_2;
        info->num++;
        dataPoints->valuerelay_2 = gizStandardDecompressionValue(relay_2_BYTEOFFSET,relay_2_BITOFFSET,relay_2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagbuzz)
    {
        info->event[info->num] = EVENT_buzz;
        info->num++;
        dataPoints->valuebuzz = gizStandardDecompressionValue(buzz_BYTEOFFSET,buzz_BITOFFSET,buzz_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagrelay_3)
    {
        info->event[info->num] = EVENT_relay_3;
        info->num++;
        dataPoints->valuerelay_3 = gizStandardDecompressionValue(relay_3_BYTEOFFSET,relay_3_BITOFFSET,relay_3_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmode_auto_manu)
    {
        info->event[info->num] = EVENT_mode_auto_manu;
        info->num++;
        dataPoints->valuemode_auto_manu = gizStandardDecompressionValue(mode_auto_manu_BYTEOFFSET,mode_auto_manu_BITOFFSET,mode_auto_manu_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmode_winter_summer)
    {
        info->event[info->num] = EVENT_mode_winter_summer;
        info->num++;
        dataPoints->valuemode_winter_summer = gizStandardDecompressionValue(mode_winter_summer_BYTEOFFSET,mode_winter_summer_BITOFFSET,mode_winter_summer_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmode_safe)
    {
        info->event[info->num] = EVENT_mode_safe;
        info->num++;
        dataPoints->valuemode_safe = gizStandardDecompressionValue(mode_safe_BYTEOFFSET,mode_safe_BITOFFSET,mode_safe_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagtemp_upper)
    {
        info->event[info->num] = EVENT_temp_upper;
        info->num++;
        dataPoints->valuetemp_upper = gizX2Y(temp_upper_RATIO,  temp_upper_ADDITION, issuedData->attrVals.valuetemp_upper); 
    }
        
    if(0x01 == issuedData->attrFlags.flagtemp_lower)
    {
        info->event[info->num] = EVENT_temp_lower;
        info->num++;
        dataPoints->valuetemp_lower = gizX2Y(temp_lower_RATIO,  temp_lower_ADDITION, issuedData->attrVals.valuetemp_lower); 
    }
        
    if(0x01 == issuedData->attrFlags.flaghumi_threshold)
    {
        info->event[info->num] = EVENT_humi_threshold;
        info->num++;
        dataPoints->valuehumi_threshold = gizX2Y(humi_threshold_RATIO,  humi_threshold_ADDITION, issuedData->attrVals.valuehumi_threshold); 
    }
        
    if(0x01 == issuedData->attrFlags.flagheater_hour)
    {
        info->event[info->num] = EVENT_heater_hour;
        info->num++;
        dataPoints->valueheater_hour = gizX2Y(heater_hour_RATIO,  heater_hour_ADDITION, issuedData->attrVals.valueheater_hour); 
    }
        
    if(0x01 == issuedData->attrFlags.flagheater_min)
    {
        info->event[info->num] = EVENT_heater_min;
        info->num++;
        dataPoints->valueheater_min = gizX2Y(heater_min_RATIO,  heater_min_ADDITION, issuedData->attrVals.valueheater_min); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuerelay_4 != cur->valuerelay_4)
    {
        GIZWITS_LOG("valuerelay_4 Changed\n");
        ret = 1;
    }
    ...
    if(last->valuerelay_2 != cur->valuerelay_2)
    {
        GIZWITS_LOG("valuerelay_2 Changed\n");
        ret = 1;
    }
    if(last->valuebuzz != cur->valuebuzz)
    {
        GIZWITS_LOG("valuebuzz Changed\n");
        ret = 1;
    }
    if(last->valuerelay_3 != cur->valuerelay_3)
    {
        GIZWITS_LOG("valuerelay_3 Changed\n");
        ret = 1;
    }
    if(last->valuemode_auto_manu != cur->valuemode_auto_manu)
    {
        GIZWITS_LOG("valuemode_auto_manu Changed\n");
        ret = 1;
    }
    if(last->valuemode_winter_summer != cur->valuemode_winter_summer)
    {
        GIZWITS_LOG("valuemode_winter_summer Changed\n");
        ret = 1;
    }
    if(last->valuemode_safe != cur->valuemode_safe)
    {
        GIZWITS_LOG("valuemode_safe Changed\n");
        ret = 1;
    }
    if(last->valuetemp_upper != cur->valuetemp_upper)
    {
        GIZWITS_LOG("valuetemp_upper Changed\n");
        ret = 1;
    }
    if(last->valuetemp_lower != cur->valuetemp_lower)
    {
        GIZWITS_LOG("valuetemp_lower Changed\n");
        ret = 1;
    }
    if(last->valuehumi_threshold != cur->valuehumi_threshold)
    {
        GIZWITS_LOG("valuehumi_threshold Changed\n");
        ret = 1;
    }
    if(last->valueheater_hour != cur->valueheater_hour)
    {
        GIZWITS_LOG("valueheater_hour Changed\n");
        ret = 1;
    }
    if(last->valueheater_min != cur->valueheater_min)
    {
        GIZWITS_LOG("valueheater_min Changed\n");
        ret = 1;
    }
    if(last->valueir != cur->valueir)
    {
        GIZWITS_LOG("valueir Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(relay_4_BYTEOFFSET,relay_4_BITOFFSET,relay_4_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerelay_4);
    gizStandardCompressValue(relay_1_BYTEOFFSET,relay_1_BITOFFSET,relay_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerelay_1);
    gizStandardCompressValue(relay_2_BYTEOFFSET,relay_2_BITOFFSET,relay_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerelay_2);
    gizStandardCompressValue(buzz_BYTEOFFSET,buzz_BITOFFSET,buzz_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebuzz);
    gizStandardCompressValue(relay_3_BYTEOFFSET,relay_3_BITOFFSET,relay_3_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerelay_3);
    gizStandardCompressValue(mode_auto_manu_BYTEOFFSET,mode_auto_manu_BITOFFSET,mode_auto_manu_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode_auto_manu);
    gizStandardCompressValue(mode_winter_summer_BYTEOFFSET,mode_winter_summer_BITOFFSET,mode_winter_summer_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode_winter_summer);
    gizStandardCompressValue(mode_safe_BYTEOFFSET,mode_safe_BITOFFSET,mode_safe_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode_safe);
    gizStandardCompressValue(ir_BYTEOFFSET,ir_BITOFFSET,ir_LEN,(uint8_t *)devStatusPtr,dataPoints->valueir);
    ...
    devStatusPtr->valuetemp_upper = gizY2X(temp_upper_RATIO,  temp_upper_ADDITION, dataPoints->valuetemp_upper); 
    devStatusPtr->valuetemp_lower = gizY2X(temp_lower_RATIO,  temp_lower_ADDITION, dataPoints->valuetemp_lower); 
    devStatusPtr->valuehumi_threshold = gizY2X(humi_threshold_RATIO,  humi_threshold_ADDITION, dataPoints->valuehumi_threshold); 
    devStatusPtr->valueheater_hour = gizY2X(heater_hour_RATIO,  heater_hour_ADDITION, dataPoints->valueheater_hour); 
    devStatusPtr->valueheater_min = gizY2X(heater_min_RATIO,  heater_min_ADDITION, dataPoints->valueheater_min); 
    devStatusPtr->valuetemp = gizY2X(temp_RATIO,  temp_ADDITION, dataPoints->valuetemp); 
    devStatusPtr->valuerh = gizY2X(rh_RATIO,  rh_ADDITION, dataPoints->valuerh); 
